		...
		if record.type == _TYPE_PTR and record.name == self.type:
			...
			try:
				...
				if not expired:
					...
					callback = lambda x: self.listener.removeService(x, self.type, record.alias, record)
					self.list.append(callback)
					return
			except:
				if not expired:
					self.services[record.alias.lower()] = record
					callback = lambda x: self.listener.addService(x, self.type, record.alias, record)
					self.list.append(callback)
